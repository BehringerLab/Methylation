---
title: "Stone et al. methylation analysis"
output:
  html_document:
    df_print: paged
---
...
WT_average_strand <- WT_average %>% 
  left_join(allCallers_WT[,1:2], by = "Position")
WT_average_strand$Strand <- as.factor(WT_average_strand$Strand)
levels(WT_average_strand$Strand) <- c("-", "+")

figure_s2 <- WT_average_strand %>% 
  drop_na() %>% 
...
WT_dm_sites <- WT_dm_sites |> 
  ...
  rowwise() |> 
  dplyr::mutate(m = CoMMA::methylBtoM(meth.diff/100, sym = TRUE))
...
ego <- simplify(ego, by = "pvalue")
go_up <- simplify(go_up, by = "pvalue")
go_dn <- simplify(go_dn, cutoff = 0.5,
                  by = "pvalue")
...
WT_gsea <- gseGO(geneList = WT_geneList_all,
                 OrgDb = org.EcK12.eg.db,
                 ont = "BP",
                 pvalueCutoff = 0.05,
                 verbose = TRUE,
...
MMR_gsea <- gseGO(geneList = MMR_geneList_all,
                 OrgDb = org.EcK12.eg.db,
                 ont = "BP",
                 pvalueCutoff = 0.05,
                 verbose = TRUE,
...
gseaplot2(WT_gsea, geneSetID = 1:5)
gseaplot2(MMR_gsea, geneSetID = 1:5)
...
edox <- setReadable(ego2, 'org.EcK12.eg.db', 'ENTREZID')
heatplot(edox, foldChange = geneList)

edox <- setReadable(ego, 'org.EcK12.eg.db', 'ENTREZID')
heatplot(edox, foldChange = geneList)
